document.addEventListener('DOMContentLoaded', function () {
    ...
    console.log('Loading Version 2');

    // Gage control json file URL
    const jsonFileURL = 'https://wm.mvs.ds.usace.army.mil/district_templates/gage_data/public/json/gage_control2.json';
    ...
    fetch(jsonFileURL)
        ...
        .then(data => {
            ...
            const filteredAndSortedData = data.reduce((accumulator, currentValue) => {
                // Filter out locations where ld_gate_summay is true
                const filteredLocations = Object.keys(currentValue)
                    .filter(key => key.startsWith('loc_'))
                    .map(key => currentValue[key])
                    .filter(location => location.ld_gate_summay === true);
                
                // Push filtered locations to the accumulator
            ...
            filteredAndSortedData.sort((a, b) => {
                const orderA = a.ld_gate_summay_sort_order || 0; // Handle cases where ld_gate_summay_sort_order is missing
